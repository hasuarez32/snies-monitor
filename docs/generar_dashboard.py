...
function plotHBar(id, rows, field, color, n) {
  const el=document.getElementById(id); if(!el) return;
  const d=[...countBy(rows,field,n||10)].reverse(); if(!d.length) return;
  Plotly.react(id,[{y:d.map(e=>e[0]),x:d.map(e=>e[1]),type:'bar',orientation:'h',
    marker:{color,opacity:.85},hovertemplate:'%{y}<br><b>%{x:,}</b><extra></extra>'}],
    {margin:{t:10,r:20,b:30,l:210},
