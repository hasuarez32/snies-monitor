...
function _applySubFilters(rows) {
  ...
  if(_cineFiltro!==null){
    const CCOL='CINE_F_2013_AC_CAMPO_ESPECÍFIC';
    const unc=_cineFiltro==='Sin clasificar';
    r=r.filter(x=>{const v=(x[CCOL]||'').trim();return unc?!v:v===_cineFiltro;});
  }
  return r;
}
...
function setPeriodosDetalleFilter(val) {
  ...
  _periodosDet=val;
  const res=_applySubFilters(filtered);
  document.getElementById('per-det-chip').style.display='flex';
  document.getElementById('per-det-val').textContent=
    val+' periodos — '+res.length.toLocaleString('es-CO')+' programas';
  renderTbl(res);
...
function clearPeriodosDetalleFilter() {
  _periodosDet=null;
  document.getElementById('per-det-chip').style.display='none';
...
function setCineFiltro(cine) {
  ...
  _cineFiltro=cine;
  const res=_applySubFilters(filtered);
  document.getElementById('cine-det-chip').style.display='flex';
  document.getElementById('cine-det-val').textContent=
    cine+' — '+res.length.toLocaleString('es-CO')+' programas';
  renderTbl(res);
  document.getElementById('tbl-wrap').scrollIntoView({behavior:'smooth'});
}
...
function clearCineFiltro() {
  _cineFiltro=null;
  document.getElementById('cine-det-chip').style.display='none';
  renderTbl(_applySubFilters(filtered));
}
...
function renderAll(rows) {
  ...
  const _cpd=document.getElementById('per-det-chip'); if(_cpd) _cpd.style.display='none';
  const _ccd=document.getElementById('cine-det-chip'); if(_ccd) _ccd.style.display='none';
